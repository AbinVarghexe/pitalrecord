...
import { IconUser, IconPill, IconCalendar, IconClock, IconAlertCircle, IconShieldCheck, IconFileText, IconEye } from '@tabler/icons-react'
import Link from 'next/link'
import type { FamilyProfile, Prescription, Medicine } from '@/lib/supabase/types'
...
export default async function DoctorSessionPage({ params }: PageProps) {
  ...
  // Get prescriptions for this profile
    ...
    <div className="min-h-screen bg-muted/30">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <Card className="mb-6">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <CardDescription className="mt-1">
                    {profile.relationship} • {profile.date_of_birth 
          ...
          {(profile.allergies?.length > 0 || profile.chronic_conditions?.length > 0) && (
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="rounded-lg border border-red-200 bg-red-50 dark:border-red-900 dark:bg-red-950/50 p-3">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {profile.allergies.map((allergy: string) => (
                        <Badge key={allergy} variant="outline" className="border-red-300 text-red-700 dark:border-red-800 dark:text-red-400">
                          {allergy}
                        </Badge>
                      ))}
                    </div>
                ...
                {profile.chronic_conditions && profile.chronic_conditions.length > 0 && (
                  <div className="rounded-lg border border-amber-200 bg-amber-50 dark:border-amber-900 dark:bg-amber-950/50 p-3">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {profile.chronic_conditions.map((condition: string) => (
                        <Badge key={condition} variant="outline" className="border-amber-300 text-amber-700 dark:border-amber-800 dark:text-amber-400">
                          {condition}
                        </Badge>
                      ))}
                    </div>
          ...
          <CardContent>
            {!prescriptions || prescriptions.length === 0 ? (
              ...
              <div className="space-y-4">
                {prescriptions.map((prescription: Prescription & { medicines: Medicine[] }) => (
                  <div key={prescription.id} className="rounded-lg border p-4">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <h4 className="font-medium">{prescription.attending_doctor || 'Unknown Doctor'}</h4>
                    ...
                    {prescription.medicines && prescription.medicines.length > 0 && (
                      <div className="mb-3">
                        <span className="text-sm font-medium mb-2 block">Medications:</span>
                        <div className="grid gap-2 sm:grid-cols-2">
                          {prescription.medicines.map((medicine: Medicine) => (
                            <div key={medicine.id} className="rounded bg-muted/50 p-2 text-sm">
